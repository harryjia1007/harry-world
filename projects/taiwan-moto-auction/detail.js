...
function render(row) {
  ...
  ${photos.length ? `<section class="detail-gallery">${photos.map((url, index) => `<a href="${M.escapeHtml(url)}" target="_blank" rel="noopener noreferrer"><img src="${M.escapeHtml(url)}" alt="${M.escapeHtml(M.title(row))} 官方照片 ${index + 1}" loading="${index ? 'lazy' : 'eager'}"></a>`).join('')}</section>` : '<p class="detail-no-photo">本站目前不顯示照片；請到官方公告確認是否有照片。本頁不使用示意圖替代。</p>'}
